...
export default () => {
    ...
    let categoryValues = {};
    let uData = userData.solves.filter(Boolean).sort((a, b) => (new Date(a.timestamp)) - (new Date(b.timestamp)));
            ...
            <div className={"userSolves"}>
                ...
                {(!userData.solves || userData.solves.filter(Boolean).length === 0) ? <div className={"noSolves"}>
                    {t("profile.no_solves", { name: userData.username })}
                        ...
                        <Tab label={"Solves"}>
                            {userData.solves && userData.solves.filter(Boolean).map((i, n) => <UserSolve key={i.timestamp} {...i} />)}
                        </Tab>
                        <Tab label={"Stats"}>
